...
if correction:
    ...
    dndM =  ncount_varied.cumsum(axis=-1)/volume/Mwidth_corr[:, np.newaxis,:]
    M_bin = M_bin_corr
    M_c = 0.5*(M_bin[:,1:] + M_bin[:,:-1])
else:
    ncount_cum = np.cumsum(ncount_varied[...,::-1],axis=-1)[...,::-1]
    dndM = ncount_cum/volume/M_width
    M_c = 0.5*(M_bin[1:] + M_bin[:-1])
...
if correction:
    train_df.to_csv('/cosma7/data/dp004/dc-cues1/DarkQuest/dataframes/train_chmf_corr.csv',index=False,)
else:
    train_df.to_csv('/cosma7/data/dp004/dc-cues1/DarkQuest/dataframes/train_chmf.csv',index=False,)
test_df = df[df['index'].isin(test_indices)]
if correction:
    test_df.to_csv('/cosma7/data/dp004/dc-cues1/DarkQuest/dataframes/test_chmf_corr.csv',index=False,)
else:
    test_df.to_csv('/cosma7/data/dp004/dc-cues1/DarkQuest/dataframes/test_chmf.csv',index=False,)
val_df = df[df['index'].isin(val_indices)]
if correction:
    val_df.to_csv('/cosma7/data/dp004/dc-cues1/DarkQuest/dataframes/val_chmf_corr.csv',index=False,)
else:
    val_df.to_csv('/cosma7/data/dp004/dc-cues1/DarkQuest/dataframes/val_chmf.csv',index=False,)

train_multi_df = pivot_k(train_df, column='dndM', column_to_pivot='mass')
if correction:
    train_multi_df.to_csv('/cosma7/data/dp004/dc-cues1/DarkQuest/dataframes/train_chmf_multi_corr.csv',index=False,)
else:
    train_multi_df.to_csv('/cosma7/data/dp004/dc-cues1/DarkQuest/dataframes/train_chmf_multi.csv',index=False,)
test_multi_df = pivot_k(test_df, column='dndM', column_to_pivot='mass')
if correction:
    test_multi_df.to_csv('/cosma7/data/dp004/dc-cues1/DarkQuest/dataframes/test_chmf_multi_corr.csv',index=False,)
else:
    test_multi_df.to_csv('/cosma7/data/dp004/dc-cues1/DarkQuest/dataframes/test_chmf_multi.csv',index=False,)
val_multi_df = pivot_k(val_df, column='dndM', column_to_pivot='mass')
if correction:
    val_multi_df.to_csv('/cosma7/data/dp004/dc-cues1/DarkQuest/dataframes/val_chmf_multi_corr.csv',index=False,)
else:
    val_multi_df.to_csv('/cosma7/data/dp004/dc-cues1/DarkQuest/dataframes/val_chmf_multi.csv',index=False,)
