...
M_bin_fit = np.logspace(12,16,401)
...
stitch_mass = pow(10,13.5)

dndM = np.concatenate((dndM_hr[...,M_c< stitch_mass], hmf_fit[...,M_bin_fit>=stitch_mass][...,::4]),axis=-1)
M_c = np.concatenate((M_c[M_c<stitch_mass], M_bin_fit[M_bin_fit>=stitch_mass][::4]))
...
train_df.to_csv('/cosma7/data/dp004/dc-cues1/DarkQuest/dataframes/train_hmf_fit_stitch.csv',index=False,)
test_df = df[df['index'].isin(test_indices)]
test_df.to_csv('/cosma7/data/dp004/dc-cues1/DarkQuest/dataframes/test_hmf_fit_stitch.csv',index=False,)
val_df = df[df['index'].isin(val_indices)]
val_df.to_csv('/cosma7/data/dp004/dc-cues1/DarkQuest/dataframes/val_hmf_fit_stitch.csv',index=False,)
