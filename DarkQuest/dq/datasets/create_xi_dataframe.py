...
def read_halo_data(r_max):
    ...
    xi_hh_fiducial = np.load(HALO_DATA_DIR / "xi/xihh_fiducial.npy")
...
def get_full_df(r_max=70., multi_output=False):
...
if __name__ == '__main__':
    ...
    train_df.to_csv('/cosma7/data/dp004/dc-cues1/DarkQuest/dataframes/train_xi_multioutput.csv',index=False,)
    test_df = df[df['index'].isin(test_indices)]
    test_df.to_csv('/cosma7/data/dp004/dc-cues1/DarkQuest/dataframes/test_xi_multioutput.csv',index=False,)
    val_df = df[df['index'].isin(val_indices)]
    val_df.to_csv('/cosma7/data/dp004/dc-cues1/DarkQuest/dataframes/val_xi_multioutput.csv',index=False,)

    df = get_full_df(multi_output=False)
    
    train_df = df[~df['index'].isin(test_indices + val_indices)]
    train_df.to_csv('/cosma7/data/dp004/dc-cues1/DarkQuest/dataframes/train_xi.csv',index=False,)
    test_df = df[df['index'].isin(test_indices)]
    test_df.to_csv('/cosma7/data/dp004/dc-cues1/DarkQuest/dataframes/test_xi.csv',index=False,)
    val_df = df[df['index'].isin(val_indices)]
    val_df.to_csv('/cosma7/data/dp004/dc-cues1/DarkQuest/dataframes/val_xi.csv',index=False,)
