...
HOD_FILE    = Path(OUTPATH / "halocat_fiducial_MGGLAM_ng_fixed.hdf5")
...
def compute_wp_of_s_z_from_HOD_catalogue(
    ...
    outfile = Path(OUTPATH / "wp_from_sz_fiducial_MGGLAM_ng_fixed.hdf5")
    if outfile.exists():
...
def compute_tpcf_from_HOD_catalogue(
    ...
    outfile = Path(OUTPATH / "tpcf_r_fiducial_MGGLAM_ng_fixed.hdf5")
    if outfile.exists():
        input(f"{outfile} exists. Press enter to overwrite.")
