...
def compute_wp_of_z_from_HOD_catalogue(
        r_binedge:  np.ndarray,
        threads:    int         = 128,
        pi_max:     float       = 200.0,
        ):
    ...
    # Create outfile for wp 
    outfile = Path(OUTPATH / "wp_from_z_fiducial_ng_fixed.hdf5")
    if outfile.exists():
        input(f"{outfile} exists. Press enter to overwrite.")
    wp_file         = h5py.File(outfile, "w")
...
compute_wp_of_z_from_HOD_catalogue(r_binedge = np.geomspace(0.5, 40, 40))
