...
"""
The script outputs hdf5 files with xi(r,C,G)-data for all simulations. 
There are several hdf5 files created from numerous functions.
Some may be seen as redundant, but they were created to:
 - Simplify adjustments of r-scales used in the end
 - Simplify process of retrieving xi-data for various analyses/testing
 - Make creation of csv files for emulation easier and *fool-proof*

The functions are:
 - make_TPCF_hdf5_files_full:
    - Creates one hdf5 file, ~/d5/TPCF_full.hdf5, with xi(r,C,G)-data for all simulations.
    - Cotains groups for train, test, val datasets.
    - Contains the entire r-range used in the TPCF calculations.
 - xi_sliced_hdf5:
    - Creates one hdf5 file, ~/d5/sliced_r/TPCF_XXX.hdf5, with xi(r,C,G)-data for each dataset (train, test, val).
    - Contains only the r-values between r_min and r_max.
    - Removes bins where xi(r) is nan or xi(r)<xi_tol.
 - xi_NOT_sliced_hdf5:
    - Creates one hdf5 file, ~/d5/TPCF_XXX.hdf5, with xi(r,C,G)-data for each dataset (train, test, val).
    - Contains the entire r-range used in the TPCF calculations.
    - Removes bins where xi(r) is nan or xi(r)<xi_tol.
 - xi_hdf5_to_csv:
    - Creates csv files from the files generated by the xi_XXX_hdf5 functions.
    - Outputs csv files to ~/d5/ or ~/d5/sliced_r/ depending on the sliced_r flag.
    - Output names are TPCF_XXX.csv. 
