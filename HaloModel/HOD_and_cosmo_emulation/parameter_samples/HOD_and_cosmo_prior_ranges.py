...
"""
Stores the prior ranges of the cosmological parameters used in the emulation.
The ranges are obtained by finding the minimum and maximum value of each parameter
by looping over all simulations in the train, test and val datasets.
Requires hdf5 files that are generated by ../make_tpcf_emulation_data_files.py 
The prior ranges are stored in a csv file, cosmo_param_limits.csv.

Once the csv file is created, the function get_cosmo_params_prior_range() can be used to retrieve the prior ranges.
HOD parameter ranges are computed on the spot as as +/- 10% of the fiducial value.

The get_X_params_prior_range() functions return a dictionary with the parameter names as keys, and the prior range as values.
These functions are used to generate the yaml file with prior ranges used in inference analysis.
"""
