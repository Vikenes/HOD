...
ng_desired = 2.174e-4
s8m_fiducial  = get_sim_params_from_csv_table(version=0, param_name="sigma8_m")
s8cb_fiducial = get_sim_params_from_csv_table(version=0, param_name="sigma8_cb")
s8_fiducial = s8m_fiducial + s8cb_fiducial
print(ng_desired / s8m_fiducial)
...
def load_ng_array_c004(flag: str = "test"):
    ...
    for i in range(4):
        s8m = get_sim_params_from_csv_table(version=i+1, param_name="sigma8_m")
        s8cb = get_sim_params_from_csv_table(version=i+1, param_name="sigma8_cb")
        s8 = s8m + s8cb
        print(f"{ng_array_mean[i] / s8m**1.5 * s8m_fiducial**1.5=:.5e}")
        # print(f"{ng_array_mean[i] / ng_desired=:.5f} | {s8m**2/s8m_fiducial**2:.5f}")
